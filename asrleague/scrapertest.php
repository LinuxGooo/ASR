...
if ($ok_to_crawl_kgs) {
    ...
    if ($pull_request != null) {
	    ...
	    cache_all_results($event_id);
            if ($debug) echo "Added game id#" . $sgf_id . " to database and updated the results cache.\n<br>";
            return;
        }
    } else {
...
function scan_kgs_player($kgsname, $event_id) {
  ...
  for ($mm = $m1; $mm <= $m2; $mm++) {
    ...
    if ($debug && ($games_added == 0)) echo "summary: no new games found.<br>\n";
    if ($debug && ($games_added == 1)) echo "summary: one new game found.<br>\n";
    if ($debug && ($games_added > 1)) echo "summary: " . $games_added . " new games found.<br>\n";
  // return what?
  return;
}
